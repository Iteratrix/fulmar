...
//!
//! The AT Protocol surface has five distinct identifier kinds — DID,
//! handle, AT URI, CID, rkey — that collapse to opaque-looking strings
//! on the wire. An rkey (`3mnsgtqsess26`) and a CID (`bafyrei...`)
//! are especially easy to substitute for one another, and a `String`
//! field can't tell them apart. These newtypes catch
//! wrong-kind-of-string at serde deserialization, before any XRPC
//! call leaves the process.
//!
//! Wire format is unchanged: each type round-trips through serde as a
//! single string. The newtype is the Rust-side discipline.
//!
...
//! intricate (RFC1035 labels, length limits, IDN); this layer is not
//! the authority. The wire is the authority; these types catch the
//! obvious wrong-kind errors.
...
pub enum IdentifierError {
    ...
    /// truncated for log hygiene (the full string may be arbitrarily
    /// long user content).
    #[error("identifier wrong kind: expected {expected}, got {got:?}")]
    WrongKind { expected: &'static str, got: String },
}

/// Truncate a string to ~60 chars for error display.
...
string_newtype! {
    ...
    /// later resolve to a different account. Anywhere stability is
    /// needed, resolve to [`Did`] first.
    Handle {
...
string_newtype! {
    ...
    /// version. Write operations (reply/like/repost) require a CID so
    /// they target the record version the caller observed, not a
    /// later edit. Most commonly base32-multibase `CIDv1`
    /// (`bafyrei...`); the older base58 form (`Qm...`) is
...
string_newtype! {
    ...
    /// a CID at a glance. Never canonical alone — always paired with
    /// a DID + collection.
    RKey {
...
mod tests {
    ...
    fn handle_accepts_dotted_no_did_prefix() {
        ...
        assert!(Handle::parse("name.example.com").is_ok());
    ...
    /// Deserializing a `Cid` from an rkey string is the exact
    /// substitution bug class these types exist to refuse.
